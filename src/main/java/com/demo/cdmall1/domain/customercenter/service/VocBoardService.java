...
//추가
	...
	public VocBoard write(VocBoardDto.Write dto, String loginId) {
		...
		Integer maxRef = dao.selectMaxRef();
		if(maxRef==null) {
			maxRef = 1;
		}else {
			maxRef++;
	...
	public VocBoard writeReply(VocBoardDto.WriteReply dto, String username) {
	    ...
	    vocBoard.setRe_seq(tempVocBoard.getRe_seq()+1);
		Jsoup.parseBodyFragment(dto.getContent()).getElementsByTag("img").forEach(img->{
			// http://localhost:8081/temp/image?imagename=aaa.jpg;
			String imageUrl = img.attr("src");
			String imageName = imageUrl.substring(imageUrl.lastIndexOf("=")+1);
		...
		vocBoard.setContent(dto.getContent().replaceAll(ZmallConstant.CK_FIND_PATTERN, ZmallConstant.CK_REPLACE_PATTERN));
		//dao.updateReSeq(tempVocBoard.getRe_ref(),tempVocBoard.getRe_seq());
		if(dto.getVattachments()==null) {
	...
	public void updateReSeq(VocBoardDto.WriteReply dto) {
		VocBoard tempVocBoard = dao.getById(dto.getVbno());
		dao.updateReSeq(tempVocBoard.getRe_ref(),tempVocBoard.getRe_seq());
	...
	public Map<String,Object> list(Integer pageno) {
		...
		Map<String,Object> map = new HashMap<>();
		map.put("content", dao.updateList(pageable));
